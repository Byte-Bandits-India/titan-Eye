...
async function findNextAvailableOptom(excludeEmails: string[]): Promise<null | { email: string; name: string }> {
  ...
  for (const optom of optomUsers) {
    if (excludeLower.includes(optom.email.toLowerCase())) {continue;}

    if (busyLower.has(optom.email.toLowerCase()) || busyLower.has(optom.name.toLowerCase())) {continue;}
...
async function reassignOrReleaseCall(id: string, customer: CustomerRow): Promise<CustomerRow | null> {
  const timestamp = new Date().toLocaleString('en-US', {
    day: 'numeric', hour: 'numeric', hour12: true, minute: '2-digit', month: 'short', second: '2-digit', year: 'numeric',
  });

  const declinedList = (customer.declinedByOptomEmails || '').split(',').map((e) => e.trim()).filter(Boolean);
...
router.get('/audit-logs', async (req: AuthenticatedRequest, res: Response) => {
  try {
    ...
    const optomUsersList = await all<{ email: string; name: string }>('SELECT email, name FROM users WHERE role = ?', ['optom']);
    const adminUsersList = await all<{ email: string; name: string }>('SELECT email, name FROM users WHERE role = ?', ['admin']);
    ...
    const isOptomActor = (takenBy?: null | string) => {
      ...
      if (lower.startsWith('dr.') || lower.includes('optom')) {return true;}
...
router.post('/', async (req: AuthenticatedRequest, res: Response) => {
  try {
    ...
    `, [
      ...
      sanitized.preferredLanguage!, sanitized.preferredLanguage2 ?? '', sanitized.storeFeedback ?? '', optomFeedbackVal,
      sanitized.status!, sanitized.activeProfile ? 1 : 0, new Date().toISOString(), sanitized.lastUpdatedOn ?? '',
...
router.put('/:id', async (req: AuthenticatedRequest, res: Response) => {
  try {
    ...
    if (c.optomFeedback === undefined) {c.optomFeedback = existingOptomFeedback;}
    ...
    if (c.optomCallStartTime === undefined) {c.optomCallStartTime = existing.optomCallStartTime;}
  ...
  try {
    ...
    if (req.user!.role === 'store') {
      ...
      if (otherActiveCustomer) {
        return res.status(409).json({ error: 'Your store already has a pending Optom request for another customer. Please wait for it to be resolved before requesting another.' });
      }
    }
...
router.post('/:id/reject-call', async (req: AuthenticatedRequest, res: Response) => {
  try {
    ...
    if (customer.status !== 'Initiated' || (customer.offeredToOptomEmail || '').toLowerCase() !== req.user!.email.toLowerCase()) {
...
setInterval(async () => {
  try {
    ...
    for (const customer of pendingOffers) {
      ...
      if (isNaN(referenceMs) || elapsedMs < OPTOM_RESPONSE_TIMEOUT_MS) {continue;}
