...
export function FeedbackScreen() {
  ...
  React.useEffect(() => {
    ...
    axios
      .get<{ customerName: string; storeName: string }>(`${API_BASE_URL}/feedback/${encodeURIComponent(token)}`)
      .then((res) => {
  ...
  return (
    <main className="flex min-h-screen w-full items-center justify-center bg-muted/40 px-4 py-10">
      <div className="w-full max-w-sm rounded-2xl border border-border bg-card p-6 shadow-lg text-center">
        ...
        {loadState === 'error' && (
          <>
            <h1 className="text-base font-bold text-foreground mb-1">Link Unavailable</h1>
            <p className="text-xs text-muted-foreground leading-relaxed">
              This feedback link is invalid or has expired. Please check with the store if you&apos;d
        ...
        {loadState === 'ready' && !submitted && (
          <>
            ...
            <Textarea
              ...
              value={feedback}
            />
            <Button className="h-9 w-full text-xs font-bold rounded-xl" disabled={isSubmitting} onClick={handleSubmit}>
        ...
        {loadState === 'ready' && submitted && (
          <>
            ...
            <h1 className="text-base font-bold text-foreground mb-1">Thank You!</h1>
            <p className="text-xs text-muted-foreground leading-relaxed">
